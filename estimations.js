document.addEventListener('DOMContentLoaded', function () {
    ...
    function saveHouseholdMembers(members) {
        ...
        if (clientIndex !== -1) {
            clients[clientIndex].householdMembers = members;
            localStorage.setItem('clients', JSON.stringify(clients));
        }
    }

    function PACEEligibilityCheck(members) {
    ...
    members.forEach(member => {
        ...
        if (age >= 65 && member["Is this person currently enrolled in PACE?"] === "no") {
            ...
            // Subtract yearly Medicare Part B Premium
            members.forEach(member => {
                ...
                if (medicarePartBExpense) {
                    ...
                    member.combinedIncome = (member.combinedIncome || 0) - yearlyMedicarePartB;
                    console.log(`Yearly Medicare Part B Premium for ${member.firstName} ${member.lastName}: $${yearlyMedicarePartB}`);
                    console.log(`Adjusted income for ${member.firstName} ${member.lastName}: $${member.combinedIncome}`);
                }
            });
    ...
    function populateHouseholdMembersUI() {
